...
<details>
<summary>Посмотрите на код ниже: 
![image](https://github.com/Comrade19632/interview/assets/42893461/2dd73262-6f1b-4b0a-ad9f-563ce30655cf)
...
<details>
<summary>Попробуйте дать определение элемента main. В чём его суть? Согласованы ли две спецификации (WHATWG and W3C) в его определении?
</summary>
<div>
  <p>
...
<details>
<summary>Зачем мы используем альтернативный текст (атрибут alt тега <img>) для изображений?
</summary>
<div>
  <p>
...
<details>
<summary>Что такое элемент canvas?
</summary>
<div>
  <p>
...
<details>
<summary>Можно ли ввести какие-то ограничения на размер текстовых полей в HTML?
</summary>
<div>
  <p>
...
<div>
  <p>Полная форма CSS — это каскадные таблицы стилей. Это язык стилей, который достаточно прост для HTML элементы. Он популярен в веб-дизайне, его применение также распространено в XHTML.
</p>
</div>
</details>
<details>
<summary>Разница между reset и normalize</summary>
<div>
  <p>Reset CSS - сброс CSS, направленный на удаление всех встроенных стилей браузера. Например: отступы, поля, размер шрифта всех элементов. Таким образом, вне зависимости от того, в каком браузере вы смотрите страницу, свойства, заданные браузерами, будут везде сброшены.</p>
  <p>Normalize CSS - стремится сделать встроенные стили браузера консистентными для всех браузеров. Также исправляются ошибки для распространенных зависимостей браузера.</p>
</div>
</details>
<details>
<summary>Каким способом я могу установить свойство opacity для элемента button, у которого есть атрибут disabled?</summary>
<div>
  <p>Через псевдо-класс :disabled.
</p>
  button:disabled {
  opacity: 0.24;
}
...
<details>
<summary>Единица измерения rem вычисляется в зависимости от значение свойства font-size элемента html. Правда или ложь?</summary>
<div>
  <p>
  Правда
</p>
...
<details>
<summary>У элемента button установлено значение 16px для свойства font-size. Как мне установить значение 8px для свойства padding в единицах измерения em?
</summary>
<div>
  <p>
...
<details>
<summary>Как рассчитывается значение свойства height у блочных элементов?
</summary>
<div>
  <p>
...
<details>
<summary>Как работает Node JS</summary>
<div>
  <ul>
     <li>
...
<details>
<summary>Можно ли реализовать многопоточность? Зачем?</summary>
<div>
  <ul>
     <li>
       Можно реализовать с помощью модуля worker_threads. Если есть необходимость выполнять какую-то сложную синхронную операцию, то такая функция блокирует поток. Если в приложении много таких функций, оно может значительно снизить производительность сервера или вообще заморозить его работу в целом. В этом случае нет способа делегировать работу пулу воркеров.
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>Какая особенность выполнения функции setImmediate. Когда будет выполнена функция обратного вызова?</summary>
<div>
  <ul>
     <li>
       Это специальный таймер, который выполняется в отдельной фазе цикла событий. Функция обратного вызова, переданная setImmediate(), будет выполнена на следующей итерации цикла событий.
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>Какая особенность выполнения функции process.nextTick()</summary>
<div>
  <ul>
     <li>
       Мы сообщаем системе о том, что эту функцию нужно вызвать после завершения текущей итерации цикла событий, до начала следующей
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>Какой вызов сработает раньше process.nextTick() или setTimeout()</summary>
<div>
  <ul>
     <li>
       process.nextTick(). Функция, переданная process.nextTick() выполнится после завершения текущей итерации цикла событий. То есть, такая функция всегда будет выполняться до функции, выполнение которой запланировано с помощью setTimeout()
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>Что сделает консольная комманда - npm install</summary>
<div>
  <ul>
     <li>
       Команда, устанавливающая пакеты представленные в package.json либо указанные после комманды
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>Чем отличается внутреннее соединение INNER JOIN (или просто JOIN) от LEFT JOIN</summary>
<div>
  <ul>
     <li>
       INNER JOIN - Этот вид джойна выведет только те строки, если условие соединения выполняется (является истинным, т.е. TRUE)
     </li>
    <li>
       LEFT JOIN — это тоже самое что и INNER JOIN (т.е. все комбинации соединений строк, отфильтрованных по какому-то условию), и плюс еще записи из левой таблицы, для которых в правой по этому фильтру ничего не совпало.
     </li>
    <br/>
  </ul>
</div>
...
<details>
<summary>В каком порядке выполняются команды запроса SQL</summary>
<div>
  <ul>
     <li>
        FROM (выбор таблицы)<br>
        WHERE (фильтрация строк)<br>
        GROUP BY (агрегирование данных)<br>
        HAVING (фильтрация агрегированных данных)<br>
...
<details>
<summary>Для чего нужны индексы</summary>
<div>
  <ul>
     <li>
        Индекс помогает быстро получить доступ к строкам требуемых данных в таблице, соответствующих запросу. Таким образом, их использование позволяет ускорить выполнение требуемого запроса.
     </li>
  </ul>
</div>
</details>
<details>
<summary>Что такое внешний ключ (Foreign key)? Какого его предназначение?</summary>
<div>
  <ul>
     <li>
...
<details>
<summary>В чем разница между командами DROP и TRUNCATE?</summary>
<div>
  <ul>
     <li>
